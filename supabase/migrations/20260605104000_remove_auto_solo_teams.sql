WITH removed_solo_teams AS (
  SELECT id
  FROM public.teams
  WHERE is_solo = true
)
DELETE FROM public.team_members tm
USING removed_solo_teams rst
WHERE tm.team_id = rst.id;
...
BEGIN
  ...
  VALUES (v_clean_name, v_invite_code, v_user_id, false)
...
BEGIN
  ...
  FROM (
    ...
    FROM (
      ...
      WHERE COALESCE(t.is_solo, false) = false
...
GRANT ALL ON FUNCTION public.create_team(text, boolean) TO authenticated, service_role;
